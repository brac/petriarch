...
const VERSION = 10; // v2:+claim; v3:+danger; v4:+passability; v5:+nutrient B; v6:+energyB; v7:+amity; v8:+scentA/B; v9:+carryState/homeGood; v10:+trail
...
export function serializeWorld(world: World): ArrayBuffer {
  ...
  //   | scentA f32[GRID] | scentB f32[GRID] | trail f32[GRID] | passability f32[GRID]
  //   | alive u8[n] | carryState u8[n] | homeGood u8[n]   (the three u8 arrays go last)
  const GRID_FIELDS = 14; // the 8 below + resourceB + resourceCapB + amity + scentA + scentB + trail
  const bytes =
...
export function restoreWorld(world: World, buf: ArrayBuffer): void {
  ...
  world.amity.set(readF32(GRID_LEN));
  world.scentA.set(readF32(GRID_LEN));
  world.scentB.set(readF32(GRID_LEN));
  world.trail.set(readF32(GRID_LEN));
